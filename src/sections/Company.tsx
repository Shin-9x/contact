...
export function Company() {
  ...
  return (
    <Section id="azienda" className="py-[clamp(56px,8vw,112px)]">
      <SectionSplit>
        <SectionLabel sticky as="h2">{t(companySection.index)}</SectionLabel>
        <SectionBody>
          <Reveal>
            ...
            <div className="mt-[clamp(40px,5vw,64px)] grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-8 border-t border-ink/10 pt-8">
              {companySection.points.map((point, index) => (
                <Reveal key={point.it} delay={90 + index * 60}>
                  <p className="text-[16px] leading-[1.55] text-ink/68">{t(point)}</p>
                </Reveal>
              ))}
            </div>
          </Reveal>
